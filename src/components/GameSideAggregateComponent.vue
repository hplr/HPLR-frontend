...
function addPlayer() {
  if (defaultGameSideObject.playerDataList !== undefined) {
    const playerId = playerSnapshot.value?.value.userId.id ?? 'default-id'; // Provide a fallback ID if undefined
...
<template>
  <div class="wrapper">
    ...
    <div class="player-list-wrapper">
      ...
      <q-btn flat label="Add player" @click="addPlayer" class="add-player-btn" />
      <div v-for="(playerKey, index) in defaultGameSideObject.playerDataList" :key="index" class="player-div">
        <PlayerComponent :player="playerKey" :player-snapshot="playerSnapshot?.value" />
      </div>
    </div>
  </div>
</template>
